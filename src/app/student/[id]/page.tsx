        ...
        return (
          <>
            ...
            <Card className="mb-6 table-wrap">
              ...
              <table><thead><tr><th>Assignment</th><th>Date</th><th>Average</th></tr></thead><tbody>{db.snapshot.assignments.map((assignment) => <tr key={assignment.id}><td>{assignment.name}</td><td>{assignment.date_administered}</td><td>{assignmentAverage(db.snapshot, assignment.id, studentId)}%</td></tr>)}</tbody></table>
            </Card>
